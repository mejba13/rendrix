    ...
    <div className="relative overflow-hidden rounded-2xl bg-white/[0.02] border border-white/[0.06]">
      ...
      <div className="relative flex flex-col items-center justify-center py-20 px-6">
        <div className="relative mb-6">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-green-500/10 flex items-center justify-center">
            <CheckCircle2 className="w-10 h-10 text-emerald-400" />
          </div>
          <div className="absolute -top-2 -right-2 w-6 h-6 rounded-lg bg-blue-500/20 flex items-center justify-center">
            <Truck className="w-3 h-3 text-blue-400" />
          </div>
        </div>

        <h3 className="text-xl font-semibold text-white mb-2">No completed orders yet</h3>
        <p className="text-white/50 text-center max-w-md mb-6">
          Completed orders will appear here once orders have been delivered to customers. These are read-only records for your reference.
        </p>
...
export default function CompletedOrdersPage() {
  ...
  const stats = React.useMemo(() => {
    if (!data?.data) return { total: 0, revenue: 0, avgOrder: 0, customers: 0 };
    const revenue = data.data.reduce((sum, o) => sum + o.total, 0);
    const uniqueCustomers = new Set(data.data.map((o) => o.customer?.id).filter(Boolean)).size;
    return {
      total: data.meta.total,
      revenue,
      avgOrder: data.meta.total > 0 ? revenue / data.meta.total : 0,
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-green-500/10 flex items-center justify-center ring-1 ring-emerald-500/20">
            <CheckCircle2 className="w-6 h-6 text-emerald-400" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Completed Orders</h1>
            <p className="text-white/50 text-sm mt-0.5">
              Successfully delivered orders with fulfillment confirmation
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            <BentoCard
              ...
              trend={stats.total > 0 ? '+15.3%' : undefined}
              trendUp={true}
            ...
            <BentoCard
              ...
              trend={stats.revenue > 0 ? '+12.8%' : undefined}
              trendUp={true}
            ...
            <BentoCard
              icon={Receipt}
              label="Average Order"
              value={formatCurrency(stats.avgOrder, 'USD')}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <InfoCard
          ...
          description="Download invoices and receipts for completed orders"
          gradient="from-blue-500 to-cyan-500"
          delay={200}
        />
        <InfoCard
          ...
          description="View shipping and delivery confirmation information"
          gradient="from-emerald-500 to-green-500"
          delay={250}
        />
        <InfoCard
          icon={FileText}
          title="Order History"
          description="Complete audit trail and status transition logs"
          gradient="from-violet-500 to-purple-500"
          delay={300}
        />
      </div>
